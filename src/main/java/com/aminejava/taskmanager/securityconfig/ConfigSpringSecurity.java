...
public class ConfigSpringSecurity {
    ...
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity, HttpServletRequest request, HttpServletResponse response) throws Exception {
        httpSecurity.cors().and().csrf().disable()

                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and()
                .addFilterAfter(jwtTokenFilter, UsernamePasswordAuthenticationFilter.class)
                .authorizeRequests(a -> a.requestMatchers("/taskmanager/v1/superadmin/management/**", "/taskmanager/v1/superadmin/profile/**").hasRole(SUPER_ADMIN.name())
                        ...
                        .requestMatchers("/taskmanager/v1/refreshToken").authenticated()).exceptionHandling(e -> e.accessDeniedHandler(accessDeniedHandler()))
